...
function init(port, applicationServerIP, applicationServerPort) {
    ...
    app.get("/check", (req, res) => {
        const P_xml = fs.readFile(__dirname + "/data/cabinetInfirmier.xml").then(dataBuffer => dataBuffer.toString());
        const P_xsd = fs.readFile(__dirname + "/data/cabinet.xsd").then(dataBuffer => dataBuffer.toString());
        const P_all = Promise.all([P_xml, P_xsd]);
        P_all.then(([str_xml, str_xsd]) => {
            // Check xml / xsd
            console.log("./data/cabinet.xsd");
            const xsdDoc = libXML.parseXml(str_xsd);
            console.log(1);
            const xmlDoc = libXML.parseXml(str_xml);
            console.log(2);
